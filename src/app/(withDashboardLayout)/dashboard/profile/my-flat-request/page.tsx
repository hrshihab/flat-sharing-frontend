...
import { Box, IconButton, Pagination, Button, Skeleton } from "@mui/material";
...
const MyFlatRequest = () => {
  ...
  const { data: flatRequestData, isLoading } = useGetFlatBookingsByUserIdQuery(
    {}
  );
  ...
  useEffect(() => {
    if (flatRequestData) {
      const updateData = flatRequestData.map((flat: any, index: number) => ({
        sl: index + 1,
        id: flat.id,
        userId: flat.userId,
        flatId: flat.flatId,
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: ({ row }: { row: { id: string } }) => (
        <Button
          onClick={() => handleStatusChange(row.id, BOOKING_STATUS.REJECTED)}
